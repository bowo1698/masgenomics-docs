...
Prior hyperparameters follow the `masbayes` v1.4.0 defaults for BayesA
($\nu = 4.5$) and BayesR
...
DT::datatable(
  final,
  options  = list(pageLength = 32, scrollX = TRUE, ordering = TRUE),
  rownames = FALSE,
...
The primary accuracy metric is $r_{\text{test},g}$, which is the correlation
between predicted GEBV and the true breeding value on the held-out
test set. In every cell of the grid the best result comes from the
marker that matches the QTL scenario.

The congruence prediction holds in all four cells. The gap is
asymmetric. SNP markers stay competitive on `QTL@SNP` continuous, with a gap of just 3%. 
Once the QTL shifts to the haplotype block, however, they lose 25-29%.
Microhaplotypes, in contrast, already encode the underlying SNP information, so they
hold up well on `QTL@SNP`. Splitting a haplotype effect
across its component SNPs dilutes the signal, and Bayesian shrinkage
trims more columns. Accuracy drops on `QTL@MH` as a result.

BayesR wins in every scenario and BayesA is usually second. Meanwhile, GBLUP and GWABLUP trail by
0.10–0.30 on $r_{\text{test},g}$. The simulated
architecture has only 10 of 100–199 columns carrying effects, which
is genuinely sparse. The BayesR mixture-of-normals with a point mass
at zero fits that shape, while the GBLUP equal-variance prior is
misspecified. GWABLUP behaves like GBLUP here because n_train = 200
is too small for the GWAS scan to produce stable per-SNP weights.
Calibration follows the same pattern: Bayesian slopes $\hat{b}$ sit
near 1, while GBLUP / GWABLUP on binary traits return $\hat{b}$
between 1.95 and 4.04, a strong under-dispersion. Posterior
heritability also tracks the truth ($\hat{h}^2_{\text{post}} =$
0.20–0.39 against target 0.3) for the Bayesian models in matched
cells.

Indeed, The dataset was intentionally small (n_train = 200, 2 SNPs/block,
10 QTL, $h^2 = 0.3$). Two caveats matter for generalising. Larger
haplotype blocks should widen the `QTL@MH` gap further. Real microhaplotypes
carry richer multi-allelic structure than the 2-SNP contrasts used here. 
A more infinitesimal architecture (many small effects
rather than 10 sparse ones) would compress the BayesR advantage over
GBLUP. These results highlight that aligning the marker unit with
the QTL unit is the largest single lever, larger than the model
choice.
